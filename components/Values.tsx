      ...
      <div className="max-w-7xl mx-auto">
        ...
        {layout === 'compact' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {values.map((value, i) => (
              <div key={value.title}>
                <p className="font-heading text-xs uppercase tracking-widest text-black/50 mb-3">0{i + 1}</p>
                ...
                <blockquote className="border-l border-black/20 pl-4">
                  ...
                  <cite className="text-xs text-black/50 not-italic mt-1 block">{value.author}</cite>
                </blockquote>
              </div>
            ))}
          </div>
              ...
              <div key={value.title} className={`grid grid-cols-1 md:grid-cols-12 gap-8 ${i > 0 ? 'pt-20 border-t border-black/10' : ''}`}>
                ...
                <div className="md:col-span-8">
                  ...
                  <blockquote className="border-l-2 border-black pl-6">
                    ...
                    <cite className="text-sm text-black/50 not-italic mt-2 block">{value.author}</cite>
                  </blockquote>
                </div>
              </div>
            ))}
